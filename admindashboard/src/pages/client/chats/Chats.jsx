...
export default function Chats() {
  ...
  const user = useSelector(selectUser);
  const experts = useSelector(selectAllCoaches) || []; // ✅ SAFE
  const chats = useSelector(getChat);
  const dispatch = useDispatch();
  ...
  useEffect(() => {
    if (user?.role === "user") {
      dispatch(getAllCoaches({ page: 1, limit: 50 }));
    }
  }, [user?.role, dispatch]);
  ...
  /* ================= SELECT EXPERT ================= */
  const chatClient = (selectedExpert) => {
    if (client) {
      socket.emit("leave_room", {
        roomId: getPrivateRoomId(user._id, client._id),
      });
    }

    socket.emit("join_room", {
      roomId: getPrivateRoomId(user._id, selectedExpert._id),
    });

    setChatClient(selectedExpert);
  };
          ...
          {experts.length === 0 ? (
            ...
            experts.map((chat) => (
              <div
                key={chat._id}
                onClick={() => chatClient(chat)}
                className="p-4 cursor-pointer hover:bg-gray-100 flex gap-3"
              >
                <div className="w-12 h-12 bg-[#D4A5A0] rounded-full flex items-center justify-center text-white">
                  {chat?.name?.[0]}
                </div>
                <div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-white border-t p-4 flex gap-2">
            ...
            <button
              ...
              className="bg-[#2D7A6D] text-white px-4 py-2 rounded-full"
            >
              <Send size={16} />
            </button>
          </div>
