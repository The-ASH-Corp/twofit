...
const Templates = () => {
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 h-[calc(100vh-130px)] bg-slate-50/50 p-4 sm:p-6 rounded-3xl overflow-hidden">
      
      {/* Header Section */}
      <div className="flex flex-col gap-6 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 shrink-0">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Broadcast Templates</h1>
            <p className="text-slate-500 text-sm font-medium mt-1">Manage and select templates for your broadcasts</p>
          </div>
      
          <div className="relative group w-full md:w-auto">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#0A4F48] transition-colors w-4 h-4" />
            <input
              type="text"
      ...
      <div className="flex-1 overflow-y-auto pr-2 no-scrollbar">
            ...
            {broadcast?.map((template, i) => (
              ...
              >
                ...
                <div className="flex flex-col gap-2 pt-1">
                  ...
                  <h3 className="text-lg font-bold text-slate-800 group-hover:text-[#0A4F48] transition-colors line-clamp-1" title={template?.title}>
                    {template?.title}
                  </h3>
                </div>
        ...
        {/* Pagination */}<div className="px-2 pb-0 pt-4 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-slate-100 mt-auto bg-transparent">
          {/* Results Per Page */}
