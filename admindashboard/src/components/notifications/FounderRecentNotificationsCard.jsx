...
import { MoreHorizontal, Bell, MessageSquare, RefreshCw, CheckCircle, AlertCircle } from "lucide-react";
import {
...
const getIcon = (iconType, className) => {
  if (iconType === "message") return <MessageSquare size={18} className={className} />;
  if (iconType === "refresh") return <RefreshCw size={18} className={className} />;
  if (iconType === "success") return <CheckCircle size={18} className={className} />;
  if (iconType === "alert") return <AlertCircle size={18} className={className} />;
      ...
      <div className="flex flex-col gap-1 md:gap-2 overflow-y-auto pr-1 md:pr-2 no-scrollbar flex-1 -mr-1 md:-mr-2">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 opacity-60">
            ...
            <p className="text-xs md:text-sm text-[#64748B] font-medium animate-pulse">Loading updates...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 md:gap-4 text-center py-6 md:py-10">
            ...
            <p className="text-xs md:text-sm text-[#64748B] font-medium">{emptyLabel}</p>
          </div>
        ) : (
          safeNotifications.map((notification, index) => {
            ...
            return (
              ...
              >
                <div className={`${style.bgClass} p-2 md:p-3 h-fit rounded-lg shrink-0 shadow-sm group-hover/item:scale-105 transition-transform duration-300`}>
                ...
                <div className="flex flex-col gap-0.5 md:gap-1 flex-1 relative">
                  ...
                  <div className="flex items-center gap-1.5 md:gap-2 mt-0.5 md:mt-1">
                   ...
                   <div className="absolute right-0 top-1 md:top-2 opacity-0 group-hover/item:opacity-100 transition-all duration-300 translate-x-2 group-hover/item:translate-x-0">
                       <CheckCircle size={12} className="text-[#0A4F48] md:w-3.5 md:h-3.5" />
                   </div>
                </div>
              </div>
            );
