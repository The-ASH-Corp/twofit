      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
             ...
             >
                <FiX size={20} className="group-hover:scale-90 transition-transform" />
             </button>
             <div>
                <h1 className="text-xl font-bold text-slate-800 tracking-tight">{heading || "Create Entry"}</h1>
                <p className="text-sm text-slate-500 font-medium">Fill in the details to create a new record</p>
             </div>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-3">
             ...
             <button 
                onClick={() => document.getElementById("base-form-submit")?.click()}
            ...
            <Form className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
              ...
              <div className="xl:col-span-2 flex flex-col gap-6">
                {fields
                  ...
                  .map((section, index) => (
                    <div key={index} className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                       <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
                          <div className="w-1 h-4 bg-[#0A4F48] rounded-full"></div>
                          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wide">{section.section}</h3>
                       </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                             {section.fields.map((field, fIndex) => (
                                <div
                                    ...
                                    className={
                                    ["radio", "checkbox-group", "time-range", "file", "textarea", "description"].includes(field.type)
                    ...
                    .map((section, index) => (
                        <div key={index} className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                            <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/30 flex items-center gap-2">
                                ...
                                <h3 className="text-xs font-bold text-slate-600 uppercase tracking-wide">{section.section}</h3>
                            </div>
