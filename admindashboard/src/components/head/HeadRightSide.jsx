    ...
    <div className="flex flex-col h-full min-h-[400px] lg:min-h-0 bg-white rounded-3xl border border-[#EEF2F6] shadow-[0_2px_12px_-4px_rgba(0,0,0,0.02)] overflow-hidden transition-all hover:shadow-[0_8px_30px_-6px_rgba(0,0,0,0.05)]">
       ...
       <div className="px-6 py-5 border-b border-[#F1F5F9] bg-[#FAFCFF] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h2 className="text-[#1E293B] font-bold text-lg tracking-tight leading-none">Performance</h2>
                <p className="text-[11px] text-[#64748B] font-medium mt-1">Key indicators & metrics</p>
            </div>
        </div>
      ...
      <div className="p-6 flex flex-col items-center justify-between flex-1 relative overflow-hidden">
        ...
        <div className="w-full grid grid-cols-3 gap-3">
            {performanceInfo.metrics.map((metric, i) => (
                <div key={i} className="group flex flex-col items-center justify-center p-3 rounded-xl bg-[#F8FAFC] border border-transparent hover:border-[#E2E8F0] hover:bg-white hover:shadow-md transition-all duration-300 relative overflow-hidden">
                    <div className={`absolute top-0 left-0 w-full h-0.5 ${metric.color} opacity-20 group-hover:opacity-100 transition-opacity`}></div>
                    
                    <span className="text-[10px] uppercase font-bold text-[#64748B] tracking-wider mb-1 group-hover:scale-105 transition-transform">{metric.label}</span>
                    <span className={`text-xl font-black tracking-tight ${metric.textColor} group-hover:scale-110 transition-transform duration-300`}>
                        {metric.value}
                    </span>
                </div>
            ))}
