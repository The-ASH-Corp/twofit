...
export default function OurBlogs() {
  ...
  useEffect(() => {
    axios
      .get("https://outstanding-stillness-production.up.railway.app/api/v1/blog/list")
      .then((res) => {
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-10'>
        {blogsData.map((blog, index) => (
          <div key={index} className='flex flex-col justify-center md:justify-between gap-3'>
            <img src={`${import.meta.env.VITE_API_URL}${blog.image}`} alt="blog image" className='rounded-lg w-full h-full'/>
