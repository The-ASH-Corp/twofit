...
export const createBroadcast = async (req, res) => {
  try {
    ...
    if (typeof req.body.variableSamples === "string") {
      try {
        req.body.variableSamples = JSON.parse(req.body.variableSamples);
      } catch (e) {
...
export const sentWhatsAppMessage = async (req, res) => {
  try {
    const data = await broadcastService.sentWhatsAppMessage(req.body);
    res.status(200).json({
      success: true,
      data
    });
...
export const getBroadcastAudience = async (req, res) => {
  try {
    ...
    const data = await broadcastService.getBroadcastAudience(page, limit, search);
